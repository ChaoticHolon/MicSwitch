...
public sealed partial class GlobalHotkeyService : IDisposable
{
    ...
    public GlobalHotkeyService(Dispatcher dispatcher, ILogger<GlobalHotkeyService> logger)
    {
    ...
    /// <summary>While true (e.g. a hotkey editor is capturing input) hotkeys are neither triggered nor suppressed.</summary>
    ...
    private static int ToVirtualKey(HotkeyGesture gesture) =>
        gesture.Key is { } name && Enum.TryParse<Key>(name, ignoreCase: true, out var key) ? KeyInterop.VirtualKeyFromKey(key) : 0;
